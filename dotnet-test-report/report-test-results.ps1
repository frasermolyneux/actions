...
<#
  ...
  This is the generic building block behind the `dotnet-test-report` composite action. It has no
  knowledge of solution/test-project layout, coverage collection, or any other repository-specific
  quality gate - callers that need those layer them on top of the base report this script returns.
#>
...
param(
    ...
    # Optional regex the repository-relative source path must match before a failure gets a
    # file/line annotation, e.g. '^src/MyProject\.Tests/.+\.cs$'. When omitted, failures are still
    # reported but without source-mapped annotations.
...
function Get-TestSource {
    ...
    if (-not $SourcePathPattern) { return $null }
...
[void]$summary.AppendLine("## $Suite tests")
